...
- [x] C3 secrets purged from the working tree AND git history — the notebook is now a {{...}} template rendered from server env, and `git filter-repo` scrubbed all 26,541 historical blobs (plaintext and base64). Tree at HEAD is byte-identical to pre-rewrite. **DONE 2026-09-16 (security-cleanup session): tree re-audited (OFF key + ntfy topic literals migrated to `ENGINE_OFF_KEY`/`BEACON_TOPIC` env — see `scripts/secret-env-migration.py` and `.env.example`), the full reachable history was rewritten with `git filter-repo` (both OFF-key values, the ntfy topic, the webhook.site token; plaintext + base64-embedded forms) and verified: 0 secret hits across all 118 rewritten commits, HEAD tree byte-identical to the verified pre-rewrite tree. Force-pushed to GitHub 2026-09-16 and verified from a fresh anonymous clone: 118 commits, 0 secret hits, single branch `main`. The stale `backup/pre-rewrite-20260916` branch (pre-rewrite root commit, 16 secret hits) was deleted from the remote. Rotation of the OFF_KEY, webhook.site token, ntfy topic and the four Kaggle keys is STILL REQUIRED and cannot be done from a sandbox: the pre-rewrite commits are already cloned, so purging does not un-leak them.**
